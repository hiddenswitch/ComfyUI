...
class LTXAVTEModel(torch.nn.Module):
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if should_use_bf16(self.execution_device):
    ...
    def load_sd(self, sd):
        if "model.layers.47.self_attn.q_norm.weight" in sd:
            ...
            sdo = state_dict_prefix_replace(sd, {"text_embedding_projection.aggregate_embed.weight": "text_embedding_projection.weight", "model.diffusion_model.video_embeddings_connector.": "video_embeddings_connector.", "model.diffusion_model.audio_embeddings_connector.": "audio_embeddings_connector."}, filter_keys=True)
            if len(sdo) == 0:
                sdo = sd
